...
- **Transaction replay prevention** -- Used transaction IDs are tracked in memory and rejected if resubmitted. Entries are pruned after one hour. Claimed IDs are released on pre-payment error paths so users can retry, but once payment is verified on-chain the ID stays claimed permanently (prevents double-spend if a later step fails). Payment transactions include a `traitswap:` note prefix to prevent cross-purpose replay between trait swap and loot box payments.
...
- **Input validation** -- All price values are guarded with `Number.isFinite()` to prevent NaN/Infinity bypass. Trait IDs are validated against a shared regex (`TRAIT_ID_REGEX`). Indexer fetches enforce a 10-second timeout. The reveal route uses a strict `appArgs.length` check to reject malformed application calls.
